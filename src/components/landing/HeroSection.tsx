...
import { exchangeIcons } from '@web3icons/react'
...
export function HeroSection({ lang, t, paymentRows }: Props) {
  ...
  return (
    <>
      ...
      <div className={cn('flex', 'mb-16', 'sm:mb-20', 'relative', 'justify-center', 'w-full')}>
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img src="/Vector 3093.svg" alt="" className={cn('w-full', 'scale-[1.05]')} />
        <Link
          href={`/${lang}/quiz/1`}
      ...
      <div className={cn('relative', 'mx-auto', 'mt-4')}>
        <div className={cn('flex', 'items-start', 'justify-between', 'gap-4', 'md:gap-6')}>
          <div className={cn('hidden', 'lg:block', 'shrink-0')}><StudentCard lang={lang} /></div>

          <div className={cn('flex-1', 'min-w-0', 'flex', 'flex-col', 'gap-4', 'md:gap-5')}>
            ...
            <div className={cn('grid', 'grid-cols-3', 'gap-2.5')}>
              {([
                [exchangeIcons.ExchangeBinance, 'Binance', 'branded' as const, ''],
                [exchangeIcons.ExchangeBybit, 'Bybit', 'branded' as const, ''],
                [exchangeIcons.ExchangeOkx, 'OKX', 'mono' as const, 'text-white'],
              ] as const).map(([Icon, name, variant, cls], i) => (
                <div key={name} className={cn('flex', 'flex-col', 'items-center', 'gap-2', 'bg-[oklch(0.14_0_0)]', 'rounded-xl', 'px-2', 'py-3')}>
          ...
          <div className={cn('hidden', 'lg:block', 'shrink-0')}><SignalCard lang={lang} /></div>
        </div>
      </div>
    </>
  )
